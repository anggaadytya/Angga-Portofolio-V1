  ...
  return (
    <div className="mt-12 md:px-[5%]">
      ...
      <a
        ...
        className="text-blue-600 dark:text-blue-300 flex gap-1 items-center hover:opacity-70 duration-100"
        rel="noreferrer"
      >
        View On Github
            ...
            {data &&
              data.user.contributionsCollection.contributionCalendar.weeks
                ...
                .map((week, index) => (
                  <ul
                    key={index}
                    className="w-full flex flex-col gap-px md:gap-1"
                  >
                    {week.contributionDays.map((contributionDay, index_) => {
                      const color =
...
async function getContributions() {
  const token = "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";
  const username = "anggaadytya";
  const headers = {
...
function getContributionsCount(data) {
  ...
  if (data != null && data != "") {
    ...
    var indexWeek =
      data.user.contributionsCollection.contributionCalendar.weeks.length - 1;
    var ccount = 0;
    for (
      var i = 0;
      i <
      data.user.contributionsCollection.contributionCalendar.weeks[indexWeek]
        .contributionDays.length;
      i++
    ) {
      ccount +=
        data.user.contributionsCollection.contributionCalendar.weeks[indexWeek]
          .contributionDays[i].contributionCount;
    }
    thisWeek = JSON.stringify(ccount);

    var indexMonth =
      data.user.contributionsCollection.contributionCalendar.months.length - 1;
    let totalWeeks =
      data.user.contributionsCollection.contributionCalendar.months[indexMonth]
        .totalWeeks;
    indexWeek =
    ...
    for (
      ...
      for (
        ...
        if (
          data.user.contributionsCollection.contributionCalendar.weeks[a]
            .contributionDays[i]
        ) {
          ccount +=
            data.user.contributionsCollection.contributionCalendar.weeks[a]
